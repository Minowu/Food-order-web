                ...
                <div class="d-flex justify-content-between">
                    <div class="info">
                        <?php if($row_sanpham['sanpham_giakhuyenmai']==0){?>
                            <p class="price"><?php echo $row_sanpham['sanpham_gia'] ?>.000 đ</p>
                        <?php
                        }else{
                        ?>
                        <p class="price"><?php echo $row_sanpham['sanpham_giakhuyenmai'] ?>.000 đ</p>
                        <p class="discount-f">
                            <span class="price-old"><?php echo $row_sanpham['sanpham_gia'] ?>.000 đ</span>
                        </p>
                        <?php }?>
                    </div>
    ...
    if (isset($_POST['id'])) {
        ...
        if (isset($_SESSION['giohang']) && !empty($_SESSION['giohang'])) {
            ...
            foreach ($_SESSION['giohang'] as &$cart_item) {
                if ($cart_item['id'] == $id) {
                    $cart_item['soluong'] += 1; // Tăng số lượng lên 1
                    $found = 1;
                    break;
                }
            }
            unset($cart_item); // Giải phóng biến tham chiếu
            
            // Nếu sản phẩm chưa có trong giỏ hàng, thêm mới
            if ($found == 0) {
                $_SESSION['giohang'][] = array('id' => $id, 'soluong' => 1); // Thêm mới với số lượng 1
            }
        } else {
            // Nếu giỏ hàng chưa tồn tại, khởi tạo giỏ hàng với sản phẩm đầu tiên
            $_SESSION['giohang'] = array(array('id' => $id, 'soluong' => 1)); // Thêm mới với số lượng 1
        }
    }
    ?>    
    </div>
